    ...
    async def validate(
        ...
        *,
        ...
        for r in regions:
            ...
            # Proxy closure-rate check across the rendezvous region's spatial
            # extent. The bbox diagonal spans two candidate meeting points; for
            # rendezvous, that span can be closed by *two* movers, so comparing
            # the diagonal speed directly to one actor's v_max over-constrains
            # valid prism intersections. We therefore bound against 2 * v_max
            # (plus tolerance) and keep this as a coarse guardrail for
            # obviously infeasible candidate regions.
            minx, miny, maxx, maxy = geom.bounds
            span_m = haversine_m(miny, minx, maxy, maxx)
            v_req_proxy = span_m / max(window_s, 1.0)
            if v_req_proxy > (2.0 * bounds.v_max_mps) * 1.05:
